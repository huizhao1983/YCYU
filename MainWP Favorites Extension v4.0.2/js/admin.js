	...
	if (type == 'plugin') {
		...
		return function (response) {
			...
			for (var siteId in response.sites) {
				...
				installQueue += '<span class="siteBulkInstall" siteid="' + siteId + '" status="queue"><strong>' + site_name + '</strong>: <span class="queue">' + __( 'Queued' ) + '</span><span class="progress"><i class="notched circle loading icon"></i> ' + __( 'In progress' ) + '</span><span class="status"></span></span><br />';
			}
			pElement.html( installQueue );
			favorite_install_bulk_start_next( pType, response.url );
                        mainwp_favorite_bulk_install_slug( type, selected_sites, selected_groups );
		}
	}(type), 'json');
...
mainwp_favorite_bulk_install = function (pType) {   
    ...
    if (pType == 'plugin') {  
        ...
        if (jQuery("div[data-tab='mainwp-favorite-plugin-groups-tab']").hasClass('active')) {
            isGroup = true;            
        } else {
            isGroup = false;            
        }
        ...
        if (isGroup) {
           jQuery( "#mainwp-favorite-plugin-groups-table tbody  .check-column INPUT:checkbox:checked" ).each(function (i) {
                ...
                var name = parent.attr('group_name').replace(/\+/g, '%20'); // plus
        ...
        jQuery('#favorites-bulk-install-modal').modal({onHide: function(){
        ...
        if (isGroup) {
            mainwp_favorite_bulk_group_install('plugin', selected_sites, selected_groups );
        } else {
            mainwp_favorite_bulk_install_slug( 'plugin', selected_sites, selected_groups );        
        }
    } else if (pType == 'theme') {
        
        if (jQuery("div[data-tab='mainwp-favorite-theme-groups-tab']").hasClass('active')) {        
            isGroup = true;            
        } else {
            isGroup = false;            
        }
        ...
        if (isGroup) {
            mainwp_favorite_bulk_group_install('theme', selected_sites, selected_groups );
        } else {
            mainwp_favorite_bulk_install_slug( 'theme', selected_sites, selected_groups );         
        }
          
    }
};

mainwp_favorite_bulk_group_install = function ( type, pSites, pGroups) {      
...
mainwp_favorite_bulk_install_slug = function ( pType, pSites, pGroups) {        
    var slugToInstall = jQuery( '.bulk-install-slug[status="queue"]:first' );
	...
	jQuery.post(ajaxurl, data, function (pType, pParams) {
		return function (response) {
			...
			group_loading_sites_start_next( pType, pParams );
                        mainwp_favorite_bulk_group_install( type, selected_sites, selected_groups );
		}
	}(type, params), 'json');	
}
