...
Documentation URI: https://mainwp.com/help/category/mainwp-extensions/favorites/
*/
...
class Favorites_Extension {
	...
	static function get_instance() {
		if ( null == Favorites_Extension::$instance ) {
			Favorites_Extension::$instance = new Favorites_Extension();
		}

		return Favorites_Extension::$instance;
	}
      ...
      if ( !wp_verify_nonce( $_POST['favor_nonce'], 'favor-nonce' ) ) {
	    ...
	    if ( isset($_POST['custom_folder'] ) && !empty( $folder ) ) {
	      ...
	      if( $old_folder != $folder ) {
	        Favorites_Extension::create_folders();
	      }
	    } else {
	  ...
	  if ( isset( $_GET['favor_mwpdl'] ) && isset( $_GET['sig'] ) ) {
	    if ( empty( $this->settings['custom_folder'] ) )
	      return;
			...
			$file   = $this->favorites_folder . rawurldecode( $_REQUEST['favor_mwpdl'] );
	...
	static function create_folders() {
		...
		$dir         = self::get_instance()->favorites_folder; //apply_filters( 'mainwp_getspecificdir', 'favorites' );
	...
	public static function validate_favories( $type ) {
		$result    = Favorites_Extension::scan_favorites_folder( $type );
		$added_fav = '';
		$messages  = array();
	...
	public static function scan_favorites_folder( $type ) {
		...
		$favorite_root  = self::get_instance()->favorites_folder . $path; //apply_filters( 'mainwp_getspecificdir', $path );
    ...
    $favorite_path = self::get_instance()->favorites_folder . $path; //apply_filters( 'mainwp_getspecificdir', $path );
	...
	public function admin_init() {
		...
		if ( isset( $_REQUEST['page'] ) ) {
			if ( 'Extensions-Mainwp-Favorites-Extension' == $_REQUEST['page'] || 'ThemesFavorite' == $_REQUEST['page'] || 'PluginsFavorite' == $_REQUEST['page'] || 'PluginsInstall' == $_REQUEST['page'] || 'ThemesInstall' == $_REQUEST['page'] ) {
				wp_enqueue_script( 'favorites-extension-admin', $this->plugin_url . 'js/admin.js', array(), '4.0'  );
		...
		if ( isset( $_REQUEST['favorites_do'] ) ) {
			if ( 'FavoritesInstallBulk-uploadfile' == $_REQUEST['favorites_do'] ) {
				...
				if ( 'plugin' == $_REQUEST['type'] ) {
					$path_type = 'plugins/';
				} else if ( 'theme' == $_REQUEST['type'] ) {
					$path_type = 'themes/';
				} else {
					$path_type = 'others/';
				}

				$path = $this->favorites_folder . $path_type; //apply_filters( 'mainwp_getspecificdir', 'favorites/' . $path_type );
	...
	public function prepare_install_group() {
		...
		foreach ( $favorites as $favorite ) {
			...
			$output['favorites'][ $favorite->id ] = self::map_favorite( $favorite, array(
				'id',
				'name',
				'download_url',
				'slug',
  ...
  public function getFavoritesDownloadUrl( $what, $filename ) {
    ...
    if ( empty( $this->settings['custom_folder'] ) ) {
      ...
      $fullFile     = $this->favorites_folder . $what . "/" . $filename;
	...
	public function favorite_removefavorite() {
		Favorites_Extension::remove_favorite();
	}

	public static function remove_favorite() {
		if ( isset( $_POST['id'] ) && Favorites_Extension::ctype_digit( $_POST['id'] ) ) {
			...
			if ( self::can_edit_favorite( $favorite ) ) {
				...
				if ( Favorites_Extension_DB::get_instance()->remove_favorite_by( 'id', $favorite->id ) ) {
					if ( 'P' == $_POST['type'] ) {
						$path = 'plugins/';
					} else if ( 'T' == $_POST['type'] ) {
						$path = 'themes/';
					}
					$path = self::get_instance()->favorites_folder . $path; //apply_filters( 'mainwp_getspecificdir', 'favorites/' . $path );
	...
	public function favorite_removegroup() {
		Favorites_Extension::remove_group();
	}
	...
	public function favorites_uploadbulkaddtofavorites() {
		...
		$favorite_path = $this->favorites_folder . $path; //apply_filters( 'mainwp_getspecificdir', $path );
...
class Favorites_Extension_Activator {
	...
	function activate_this_plugin() {
		...
		add_filter( 'mainwp-getsubpages-themes', array( &$this, 'get_sub_pages_themes' ) );
		add_filter( 'mainwp-getsubpages-plugins', array( &$this, 'get_sub_pages_plugins' ) );
