...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Markup' ) ) {
	...
	class BSF_AIOSRS_Pro_Markup {
		...
		public function schema_markup() {
			...
			$current_post_id = get_the_id();
			$json_ld_markups = get_option( 'wp_schema_pro_optimized_structured_data', array() );
			if ( isset( $json_ld_markups[ $current_post_id ] ) ) {
				echo $json_ld_markups[ $current_post_id ]; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				return;
			}
			$result = self::get_schema_posts();
			if ( is_array( $result ) && ! empty( $result ) ) {
				...
				$json_ld_markups[ $current_post_id ] = $json_ld_markup;
				echo $json_ld_markup; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				update_option( 'wp_schema_pro_optimized_structured_data', $json_ld_markups );
			}
		}
