...
class MainWP_Comment {
					...
					<div class="ui mini form">
						...
						<div class="field">
							<div class="ui input fluid">
								<input type="text" placeholder="<?php esc_attr_e( 'Containing keyword', 'mainwp-comments-extension' ); ?>" id="mainwp_comment_search_by_keyword" class="text" value="<?php if ( null != $cachedSearch ) { echo $cachedSearch['keyword']; } ?>" />
							</div>
						</div>
						<div class="field">
							...
							<div class="two fields">
								<div class="field ui calendar">
									<div class="ui input left icon">
										...
										<input type="text" placeholder="<?php esc_attr_e( 'Date', 'mainwp-comments-extension' ); ?>" id="mainwp_comment_search_by_dtsstart" value="<?php if ( $cachedSearch != null ) { echo esc_attr( $cachedSearch[ 'dtsstart' ]); } ?>"/>
									</div>
								</div>
								<div class="field ui calendar">
									<div class="ui input left icon">
										<i class="calendar icon"></i>
										<input type="text" placeholder="<?php esc_attr_e( 'Date', 'mainwp-comments-extension' ); ?>" id="mainwp_comment_search_by_dtsstop" value="<?php if ( $cachedSearch != null ) { echo esc_attr( $cachedSearch[ 'dtsstop' ]); } ?>"/>
									</div>
								</div>
							</div>
						</div>
	...
	public static function render_table( $keyword, $dtsstart, $dtsstop, $status, $groups, $sites, $postId ) {
		...
		$dbwebsites = apply_filters( 'mainwp-getdbsites', $mainwpCommentsExtensionActivator->get_child_file(), $mainwpCommentsExtensionActivator->get_child_key(), $sites, $groups );
		...
		if ( count( $dbwebsites ) > 0 ) {
			...
			do_action( 'mainwp_fetchurlsauthed', $mainwpCommentsExtensionActivator->get_child_file(), $mainwpCommentsExtensionActivator->get_child_key(), $dbwebsites, 'get_all_comments', $post_data, array( MainWP_Comment::get_class_name(), 'comments_search_handler' ), $output );
		}
...
//			$comments = unserialize( base64_decode( $results[1] ) );
			...
			foreach ( $comments as $comment ) {
				...
				<tr class="mainwp-comment-item">
					...
					<td>
						...
						<?php echo $comment['author_email']; ?><br/>
					...
					<td>
						<strong><?php echo MainWP_Comment::get_status( $comment['status'] ); ?></strong><br/>
						...
						<div class="ui right pointing dropdown icon mini basic green button" style="z-index:999;">
							...
							<div class="menu">
								...
								<?php if ( $comment[ 'status' ] != 'trash' && $comment[ 'status' ] != 'spam' ) : ?>
							  	<a class="item" href="admin.php?page=SiteOpen&websiteid=<?php echo $website->id; ?>&location=<?php echo base64_encode( 'comment.php?action=editcomment&c=' . $comment['id'] ); ?>"><?php _e( 'Edit', 'mainwp-comments-extension' ); ?></a>
	...
	function mainwp_comments_search() {
		MainWP_Comment::render_table( $_POST['keyword'], $_POST['dtsstart'], $_POST['dtsstop'], $_POST['status'], ( isset( $_POST['groups'] ) ? $_POST['groups'] : '' ), ( isset( $_POST['sites'] ) ? $_POST['sites'] : '' ), ( isset( $_POST['postId'] ) ? $_POST['postId'] : '' ) );
		die();
	}
