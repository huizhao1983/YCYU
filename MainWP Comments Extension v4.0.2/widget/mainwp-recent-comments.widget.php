...
class MainWP_Recent_Comments {
	...
	public static function render() {
		MainWP_Recent_Comments::render_sites( false, false );
	}
				...
				for ( $i = 0; $i < count( $recent_comments_approved ) && $i < 5; $i ++ ) {
					...
					<div class="item mainwp-comment-item">
						<div class="ui grid">
							...
							<div class="four wide middle aligned column">
								<a href="<?php echo $recent_comments_approved[ $i ]['website']->url; ?>" target="_blank"><?php echo MainWP_Comment_Utility::get_nice_url( $recent_comments_approved[ $i ]['website']->url ); ?></a>
							</div>
								...
								<div class="ui right pointing dropdown icon mini basic green button" style="z-index: 999;">
									<i class="ellipsis horizontal icon"></i>
										<div class="menu">
											...
											<a class="item" href="admin.php?page=SiteOpen&websiteid=<?php echo $recent_comments_approved[ $i ]['website']->id; ?>&location=<?php echo base64_encode( 'comment.php?action=editcomment&c=' . $recent_comments_approved[ $i ]['id'] ); ?>"><?php _e( 'Edit', 'mainwp-comments-extension' ) ?></a>
				...
				for ( $i = 0; $i < count( $recent_comments_pending ) && $i < 5; $i ++ ) {
					...
					<div class="item mainwp-comment-item">
						<div class="ui grid">
							...
							<div class="four wide middle aligned column">
								<a href="<?php echo $recent_comments_pending[ $i ]['website']->url; ?>" target="_blank"><?php echo MainWP_Comment_Utility::get_nice_url( $recent_comments_pending[ $i ]['website']->url ); ?></a>
							</div>
							<div class="two wide column right aligned middle aligned">
								<div class="ui right pointing dropdown icon mini basic green button" style="z-index: 999;">
									<i class="ellipsis horizontal icon"></i>
										<div class="menu">
											...
											<a class="item" href="admin.php?page=SiteOpen&websiteid=<?php echo $recent_comments_pending[ $i ]['website']->id; ?>&location=<?php echo base64_encode( 'comment.php?action=editcomment&c=' . $recent_comments_pending[ $i ]['id'] ); ?>"><?php _e( 'Edit', 'mainwp-comments-extension' ) ?></a>
											<a class="item comment_submitdelete" href="#"><?php _e( 'Trash', 'mainwp-comments-extension' ); ?></a>
	...
	protected static function action( $pAction ) {
		...
		if ( isset( $_POST['comment_ids'] ) && $_POST['comment_ids'] ) {
			MainWP_Recent_Comments::bulk_action( $pAction );
			return;
		}
		...
		$information = apply_filters('mainwp_fetchurlauthed', $mainwpCommentsExtensionActivator->get_child_file(), $mainwpCommentsExtensionActivator->get_child_key(), $websiteId, 'comment_action', array(
			'action' => $pAction,
			'id' => $commentId,
		));
	...
	protected static function bulk_action( $pAction ) {
		...
		if ( count( $websites ) > 0 ) {
			foreach ( $websites as $wpid => $web ) {
				...
				$information = apply_filters( 'mainwp_fetchurlauthed', $mainwpCommentsExtensionActivator->get_child_file(), $mainwpCommentsExtensionActivator->get_child_key(), $wpid, 'comment_bulk_action', array(
					'action' => $pAction,
					'ids' => $comm_ids,
				) );
		...
		$ret['message'] = $success .' ' . MainWP_Recent_Comments::action_message( $pAction );
